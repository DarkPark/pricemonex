...
$mbox_uri  = '{imap.pochta.ru/imap/ssl/novalidate-cert}&BBAEQARFBDgEMg-/NTCom';
...
$time_start = time();
$log_buffer = '';
$log_file   = app_path . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . 'ntcom.' . date('Ym') . '.log';
...
// db connection
try {
	$dbh = new PDO('sqlite:' . app_path . DIRECTORY_SEPARATOR . 'db' . DIRECTORY_SEPARATOR . 'ntcom.sqlite');
        ...
        if ( !isset($updates[$msg_id]) ) {
			...
			if ( isset($structure->parts) && $structure->parts ) {
				foreach ( $structure->parts as $part_num => $part ) {
					if ( $part->subtype == 'X-ZIP-COMPRESSED' ) {
						$filename = $part->parameters[0]->value;
						// need to make sure it's a price
						if ( stripos($filename, 'NTCom') !== false ) {
							$updates_count++;
							mlog("File: $filename");
							mlog("\tid :: " . $msg_id);
...
if ( $list_files ) {
	...
	foreach ( $list_files as $file_data ) {
		...
		rename($filename, app_path . DIRECTORY_SEPARATOR . 'public' . DIRECTORY_SEPARATOR . 'xls' . DIRECTORY_SEPARATOR . 'ntcom.xls');
	}
}
...
function & GetXlsData ( $filename ) {
	...
	for ( $i = 8; $i <= $xls->sheets[0]['numRows']; $i++ ) {
		...
		if ( count($row) == 1 ) {
			...
			if ( $row[1] && $row[3] ) {
				$xls_data[$section][trim($row[1])] = array(
					'art'      => trim($row[2]),
					'name'     => trim($row[3]),
					'price'    => trim($row[5]),
					'warranty' => trim($row[4]) == '-' ? 0 : trim($row[4]),
				);
			}
		}
	}
