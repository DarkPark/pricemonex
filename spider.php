...
$time_start = time();
$log_buffer = '';
$log_file   = app_path . DIRECTORY_SEPARATOR . 'logs' . DIRECTORY_SEPARATOR . date('Ym') . '.log';
...
// db connection
try {
	$dbh = new PDO('sqlite:' . app_path . DIRECTORY_SEPARATOR . 'db' . DIRECTORY_SEPARATOR . 'pricemon.sqlite');
...
// check mail
if ( ($mbox = imap_open($mbox_uri, $mbox_user, $mbox_pass)) ) {
	// iterate messages
			...
			foreach ( $xls_data as $section_name => $section_items ) {
				...
				if ( $section_items ) {
					foreach ( $section_items as $item_id => $item_data ) {
						$item_id = get_item_id($item_id, $section_id, $update_id, $item_data['name'], $item_data['price'], $item_data['warranty']);
...
function get_item_id ( $id, $sid, $uid, $name, $price, $warranty ) {
	global $dbh, $list_items, $list_items_prev, $items_added, $items_updated;
	
	if ( !isset($list_items[$id]) ) {
		$dbh->query(sprintf("insert into items (id, id_section, id_update, name, price, price_diff, warranty, is_new) values (%s, %s, %s, '%s', %s, 0, %s, 1)", $id, $sid, $uid, $name, $price, $warranty));
		$items_added++;
		$list_items[$id] = true;
	} else {
		$is_new = isset($list_items_prev[$id]) ? 0 : 1;
		$dbh->query(sprintf("update items set id_section = %s, name = '%s', price = %s, price_diff = '%s' - price, id_update = %s, warranty = '%s', is_new = %s where id = '%s'", $sid, $name, $price, $price, $uid, $warranty, $is_new, $id));
		$items_updated++;
	}
	return $id;
}
